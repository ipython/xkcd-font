...
[ "$FROM" -le 6 ] && $RUN_CTXT python3 pt6_derived_chars.py xkcd-mono
[ "$FROM" -le 7 ] && $RUN_CTXT python3 pt7_font_properties.py
[ "$FROM" -le 7 ] && $RUN_CTXT python3 pt7_font_properties.py xkcd-mono
...
[ "$FROM" -le 8 ] && $RUN_CTXT python3 pt8_gen_reprod_font.py xkcd-mono
